...
def main():
    ...
    neutral = np.load('./dataset/neutral.npy')
    sad = np.load('./dataset/sad.npy')

    happy_labels = np.full((happy.shape[0], 1), 0)
    neutral_labels = np.full((neutral.shape[0], 1), 1)
    sad_labels = np.full((sad.shape[0], 1), 2)
    ...
    images = np.vstack((happy, neutral, sad))
    labels = np.vstack((happy_labels, neutral_labels, sad_labels))
